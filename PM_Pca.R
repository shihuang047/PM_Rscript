...
# Last update: 2016-09-22,Zheng Sun, Yanhai Gong, Xiaoquan Su
#################################################################
...
PM_pca <- function(da, md) {
  ...
  for (i in 1:n) {
    ...
    if (opts$drawlabel) {
      pca12<-ggplot() +
        geom_point(data=data,aes(x=PC1,y=PC2,color=group,label=group2,ymax=max(PC2)),stat='identity',position='stack',size=ps) + 
      ...
      pca13<-ggplot() +
        geom_point(data=data,aes(x=PC1,y=PC3,colour=group,label=group2,ymax=max(PC3)),stat='identity',position='stack',size=ps) +
      ...
      pca23<-ggplot() +
        geom_point(data=data,aes(x=PC2,y=PC3,colour=group,label=group2,ymax=max(PC3)),stat='identity',position='stack',size=ps) +
